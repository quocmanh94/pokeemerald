...
static void Task_RareCandyEvoAtMaxLevel(u8);
...
static bool8 IsMultiBattlePartnerSlot(u8 slot)
{
...
{
    if (gPartyMenu.layout == PARTY_LAYOUT_MULTI && IsMultiBattlePartnerSlot(slot))
        return TRUE;
...
void ItemUseCB_Medicine(u8 taskId, TaskFunc task)
{
    ...
    if (IsMultiBattle() == TRUE && IsMultiBattlePartnerSlot(gPartyMenu.slotId))
    {
        cannotUse = TRUE;
    }
...
static void TryUsePPItem(u8 taskId)
{
    ...
    if (IsMultiBattle() == TRUE && IsMultiBattlePartnerSlot(ptr->slotId))
    {
        gPartyMenuUseExitCallback = FALSE;
        PlaySE(SE_SELECT);
        DisplayPartyMenuMessage(gText_WontHaveEffect, TRUE);
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    else
    {
        ...
        if (targetSpecies != SPECIES_NONE)
        {
            ...
            DisplayPartyMenuMessage(gStringVar4, TRUE);
            ScheduleBgCopyTilemapToVram(2);
            gTasks[taskId].func = Task_RareCandyEvoAtMaxLevel;
            return;
        }
...
static void Task_RareCandyEvoAtMaxLevel(u8 taskId)
{
    if (WaitFanfare(FALSE) && IsPartyMenuTextPrinterActive() != TRUE && JOY_NEW(A_BUTTON | B_BUTTON))
    {
        PlaySE(SE_SELECT);
...
static bool8 TrySwitchInPokemon(void)
{
    ...
    if (IsMultiBattle() == TRUE && IsMultiBattlePartnerSlot(slot))
    {
