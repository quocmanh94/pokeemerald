...
u16 MonTryLearningNewMoveEvolution(struct Pokemon *mon, bool8 firstMove)
{
    ...
    while (gLevelUpLearnsets[species][sLearningMoveTableID] != LEVEL_UP_END)
    {
        ...
        if (moveLevel == 0 || moveLevel == (level << 9))
        {
            gMoveToLearn = gLevelUpLearnsets[species][sLearningMoveTableID] & LEVEL_UP_MOVE_ID;
            sLearningMoveTableID++;
    ...
    case EVO_MODE_NORMAL:
    {
        ...
        for (i = 0; i < EVOS_PER_MON; i++)
        {
            ...
            case EVO_FRIENDSHIP_DAY:
                if (timeOfDay == TIME_OF_DAY_DAY && friendship >= FRIENDSHIP_EVO_THRESHOLD)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_FRIENDSHIP_NIGHT:
                if (timeOfDay == TIME_OF_DAY_NIGHT && friendship >= FRIENDSHIP_EVO_THRESHOLD)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL:
            ...
            case EVO_LEVEL_FEMALE:
                ...
                break;
            case EVO_LEVEL_DAY:
                if (gEvolutionTable[species][i].param <= level && timeOfDay == TIME_OF_DAY_DAY)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL_NIGHT:
                if (gEvolutionTable[species][i].param <= level && timeOfDay == TIME_OF_DAY_NIGHT)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL_MORNING:
                if (gEvolutionTable[species][i].param <= level
                 && hour >= TIME_NIGHT_END_HOUR
                 && hour < TIME_MORNING_TWILIGHT_END_HOUR)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL_DUSK:
                if (gEvolutionTable[species][i].param <= level
                 && hour >= TIME_DAY_END_HOUR
                 && hour < TIME_NIGHT_START_HOUR)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_HOLD_ITEM:
            ...
            case EVO_HOLD_ITEM_DAY:
                if (gEvolutionTable[species][i].param == heldItem && timeOfDay == TIME_OF_DAY_DAY)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_HOLD_ITEM_NIGHT:
                if (gEvolutionTable[species][i].param == heldItem && timeOfDay == TIME_OF_DAY_NIGHT)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_MOVE:
            {
                ...
                {
                    if (GetMonData(mon, MON_DATA_MOVE1 + moveSlot) == gEvolutionTable[species][i].param)
                    {
                        targetSpecies = gEvolutionTable[species][i].targetSpecies;
                        break;
                    }
            ...
            case EVO_MON_IN_PARTY:
            {
                ...
                for (partyIndex = 0; partyIndex < gPlayerPartyCount; partyIndex++)
                {
                    if (!GetMonData(&gPlayerParty[partyIndex], MON_DATA_IS_EGG)
                     && GetMonData(&gPlayerParty[partyIndex], MON_DATA_SPECIES) == gEvolutionTable[species][i].param)
                    {
                        targetSpecies = gEvolutionTable[species][i].targetSpecies;
                        break;
                    }
            ...
            case EVO_LEVEL_LOCATION:
                if (gEvolutionTable[species][i].param == gMapHeader.regionMapSectionId)
                    targetSpecies = gEvolutionTable[species][i].targetSpecies;
                break;
            case EVO_LEVEL_RAIN:
                if (gEvolutionTable[species][i].param <= level
            ...
            case EVO_LEVEL_DARK_TYPE_IN_PARTY:
            {
                ...
                for (partyIndex = 0; partyIndex < gPlayerPartyCount; partyIndex++)
                {
                    u16 partySpecies = GetMonData(&gPlayerParty[partyIndex], MON_DATA_SPECIES);

                    if (!GetMonData(&gPlayerParty[partyIndex], MON_DATA_IS_EGG)
                     && partySpecies != SPECIES_NONE
                     && (gSpeciesInfo[partySpecies].types[0] == TYPE_DARK
                      || gSpeciesInfo[partySpecies].types[1] == TYPE_DARK))
                    {
                        targetSpecies = gEvolutionTable[species][i].targetSpecies;
                        break;
                    }
...
struct MoveRelearnerContext
{
    const u16 *learnedMoves;
    u16 *moves;
    bool8 *visitedSpecies;
    u8 numMoves;
    u8 preEvolutionLevel;
};

static void AddMoveRelearnerMovesForSpecies(u16 species, u8 level, struct MoveRelearnerContext *context)
{
    s32 i;
    s32 j;

    if (species == SPECIES_NONE || species >= NUM_SPECIES || context->visitedSpecies[species])
        return;

    context->visitedSpecies[species] = TRUE;
    for (i = 0; i < MAX_LEVEL_UP_MOVES && gLevelUpLearnsets[species][i] != LEVEL_UP_END; i++)
    {
        ...
        for (j = 0; j < MAX_MON_MOVES; j++)
        {
            if (context->learnedMoves[j] == move)
                break;
        }
        if (j != MAX_MON_MOVES)
            continue;

        for (j = 0; j < context->numMoves; j++)
        {
            if (context->moves[j] == move)
                break;
        }
        if (j != context->numMoves)
            continue;

        if (context->numMoves == MAX_RELEARNER_MOVES)
            return;

        context->moves[context->numMoves++] = move;
    }

    // Include every possible parent in species order for deterministic branched evolution handling.
    for (i = 1; i < NUM_SPECIES && context->numMoves < MAX_RELEARNER_MOVES; i++)
    {
        for (j = 0; j < EVOS_PER_MON; j++)
        {
            if (gEvolutionTable[i][j].targetSpecies == species)
            {
                AddMoveRelearnerMovesForSpecies(i, context->preEvolutionLevel, context);
                break;
            }
        }
    }
...
{
    bool8 visitedSpecies[NUM_SPECIES] = {FALSE};
    u16 learnedMoves[MAX_MON_MOVES];
    u16 species = GetMonData(mon, MON_DATA_SPECIES_OR_EGG, 0);
    u8 level = GetMonData(mon, MON_DATA_LEVEL, 0);
    struct MoveRelearnerContext context =
    {
        .learnedMoves = learnedMoves,
        .moves = moves,
        .visitedSpecies = visitedSpecies,
        .numMoves = 0,
        .preEvolutionLevel = level > PRE_EVOLUTION_MOVE_LEVEL_OFFSET
            ? level - PRE_EVOLUTION_MOVE_LEVEL_OFFSET
            : 1,
    };
    s32 i;
    ...
    AddMoveRelearnerMovesForSpecies(species, level, &context);
    return context.numMoves;
}
...
// Only B_POSITION_OPPONENT_LEFT is used by the summary screen. Keep the positions up
// to that index and all four frames required by Castform.

#define ALLOC_FAIL_BUFFER (1 << 0)
#define ALLOC_FAIL_STRUCT (1 << 1)
#define GFX_MANAGER_ACTIVE 0xA3 // Arbitrary value
#define GFX_MANAGER_NUM_SPRITES (B_POSITION_OPPONENT_LEFT + 1)
...
struct MonSpritesGfxManager *CreateMonSpritesGfxManager(void)
{
    ...
    gfx->numSprites = GFX_MANAGER_NUM_SPRITES;
    gfx->numFrames = MAX_MON_PIC_FRAMES;
    gfx->spriteBuffer = AllocZeroed(MON_PIC_SIZE * gfx->numFrames * gfx->numSprites);
    gfx->spritePointers = AllocZeroed(sizeof(*gfx->spritePointers) * gfx->numSprites);
    ...
    {
        for (i = 0; i < gfx->numSprites; i++)
            gfx->spritePointers[i] = gfx->spriteBuffer + (MON_PIC_SIZE * gfx->numFrames * i);
    }
