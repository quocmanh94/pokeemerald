...
static const u8 sContextMenuItems_BallsPocketNoUse[] = {
...
static const u8 sRegisteredSelectLongMale_Gfx[] = INCGFX_U8("graphics/bag/select_button_hold.png", ".4bpp");
static const u8 sRegisteredSelectLongFemale_Gfx[] = INCGFX_U8("graphics/bag/select_button_hold_female.png", ".4bpp");
...
static void BagMenu_ItemPrintCallback(u8 windowId, u32 itemIndex, u8 y)
{
    ...
    if (itemIndex != LIST_CANCEL)
    {
        ...
        else
        {
            ...
            {
                const u8 *gfx = (!IsWallysBag() && gSaveBlock2Ptr->playerGender != MALE)
                    ? sRegisteredSelectLongFemale_Gfx
                    : sRegisteredSelectLongMale_Gfx;

                BlitBitmapToWindow(windowId, gfx, 96, y - 1, 24, 16);
            }
        }
    }
}
    ...
    default:
        ...
        else
        {
            ...
            case BALLS_POCKET:
                ...
                {
                    gBagMenu->contextMenuItemsPtr = sContextMenuItems_BallsPocketNoUse;
                    gBagMenu->contextMenuNumItems = ARRAY_COUNT(sContextMenuItems_BallsPocketNoUse);
                }
                else
                {
...
static void ItemMenu_Register(u8 taskId)
{
    ...
    else
    {
        *registerSlot = gSpecialVar_ItemId;
        if (tIsRegisterHold && gSaveBlock1Ptr->registeredItem == gSpecialVar_ItemId)
            gSaveBlock1Ptr->registeredItem = ITEM_NONE;
        else if (!tIsRegisterHold && gSaveBlock1Ptr->registeredLongItem == gSpecialVar_ItemId)
...
static void AddRegisterSubMenu(void)
{
    ...
    StringExpandPlaceholders(gStringVar4, gText_RegisterHow);
    FillWindowPixelBuffer(WIN_DESCRIPTION, PIXEL_FILL(0));
    BagMenu_Print(WIN_DESCRIPTION, FONT_NORMAL, gStringVar4, 3, 1, 0, 0, 0, COLORID_NORMAL);
    PrintContextMenuItemGrid(BagMenu_AddWindow(ITEMWIN_2x2), 2, 2);
}
...
{
    u8 taskId;
    u16 *registeredItem = isRegisterHold ? &gSaveBlock1Ptr->registeredLongItem : &gSaveBlock1Ptr->registeredItem;
