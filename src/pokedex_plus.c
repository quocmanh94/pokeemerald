...
{
    CopyToBgTilemapBuffer(1, sPokedexPlus_ScreenSelectBarSubmenu_Tilemap, 0, 0);
    CopyBgTilemapBufferToVram(1);
}
...
void Task_DisplayCaughtMonDexPagePlus(u8 taskId)
{
    ...
    case 6:
        if (!gPaletteFade.active)
        {
            PlayCry_Normal(NationalPokedexNumToSpeciesPlus(dexNum), 0);
...
static void Task_LoadStatsScreen(u8 taskId)
{
    ...
    case 5:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            LoadMonIconPalette(NationalPokedexNumToSpeciesPlus(sPokedexListItem->dexNum)); // Loads pallete for current mon
            gTasks[taskId].data[4] = CreateMonIcon(NationalPokedexNumToSpeciesPlus(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18, 31, 4, 0, TRUE); // Create pokemon sprite
...
static void Task_LoadEvolutionScreen(u8 taskId)
{
    ...
    case 3:
        if (gTasks[taskId].data[1] == 0)
        {
            ...
            FreeMonIconPalettes(); // Free space for new pallete
            LoadMonIconPalette(NationalPokedexNumToSpeciesPlus(sPokedexListItem->dexNum)); // Loads pallete for current mon
            PrintPreEvolutions(taskId, NationalPokedexNumToSpeciesPlus(sPokedexListItem->dexNum));
            gTasks[taskId].data[4] = CreateMonIcon(NationalPokedexNumToSpeciesPlus(sPokedexListItem->dexNum), SpriteCB_MonIcon, 18 + 32 * sPokedexView->numPreEvolutions, 31, 4, 0, TRUE); // Create pokemon sprite
    ...
    if (sPokedexView->sEvoScreenData.numAllEvolutions != 0 && sPokedexView->sEvoScreenData.numSeen != 0)
    {
        ...
        else if (JOY_NEW(DPAD_DOWN))
        {
            while (TRUE)
            {
                pos += 1;
                if (pos >= max)
                    pos = sPokedexView->sEvoScreenData.menuPos;
