...
static u8 GetTrainerClassBall(u8 trainerClass);
...
EWRAM_DATA bool8 gPlayerDoesNotWantToEvolve[MAX_BATTLERS_COUNT] = {FALSE};
...
EWRAM_DATA u16 gLastThrownBall = ITEM_NONE;
EWRAM_DATA u16 gBallToDisplay = ITEM_NONE;
...
static const u8 sTrainerClassBallTable[TRAINER_CLASS_RS_PROTAG + 1] =
{
...
static void CB2_InitBattleInternal(void)
{
    ...
    for (i = 0; i < MAX_BATTLERS_COUNT; i++)
        gPlayerDoesNotWantToEvolve[i] = FALSE;
}
...
static u8 GetTrainerClassBall(u8 trainerClass)
{
    if (trainerClass <= TRAINER_CLASS_RS_PROTAG && sTrainerClassBallTable[trainerClass] != ITEM_NONE)
        return sTrainerClassBallTable[trainerClass];
...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum, bool8 firstTrainer)
{
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            ball = GetTrainerClassBall(gTrainers[trainerNum].trainerClass);
...
void BattleTurnPassed(void)
{
    ...
    *(&gBattleStruct->absentBattlerFlags) = gAbsentBattlerFlags;
    BattlePutTextOnWindow(gText_EmptyString3, B_WIN_MSG);
    if (gSaveBlock2Ptr->optionsMidBattleEvolutionOff)
        gBattleMainFunc = HandleTurnActionSelectionState;
    else
...
static void PlayerTryEvolution(void)
{
    ...
    for (battler = 0; battler < gBattlersCount; battler++)
    {
        if (GetBattlerSide(battler) != B_SIDE_PLAYER
            ...
            || gPlayerDoesNotWantToEvolve[battler])
            continue;

        partyId = gBattlerPartyIndexes[battler];
        if (!(gLeveledUpInBattle & gBitTable[partyId]))
            continue;

        gLeveledUpInBattle &= ~(gBitTable[partyId]);
        species = GetEvolutionTargetSpecies(&gPlayerParty[partyId], EVO_MODE_NORMAL, ITEM_NONE);
        if (species != SPECIES_NONE)
        {
            u8 taskId;
...
u32 GetBattlerTotalSpeed(u8 battler)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        if ((gBattleMons[battler].ability == ABILITY_SWIFT_SWIM && gBattleWeather & B_WEATHER_RAIN)
            ...
            || (gBattleMons[battler].ability == ABILITY_SAND_RUSH && gBattleWeather & B_WEATHER_SANDSTORM))
            speedMultiplier = 2;
    }
...
{
    u8 strikesFirst = 0;
    u32 speedBattler1 = GetBattlerTotalSpeed(battler1);
    u32 speedBattler2 = GetBattlerTotalSpeed(battler2);
    u8 holdEffect;
    u8 holdEffectParam;
