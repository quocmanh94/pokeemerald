...
#define ORAS_DOWSING_SOUNDS       TRUE
#define ORAS_DOWSING_COLOR_PAL    15
#define ORAS_DOWSING_COLOR_NONE   RGB(15, 15, 15)
#define ORAS_DOWSING_COLOR_SLOW   RGB(7, 15, 31)
#define ORAS_DOWSING_COLOR_NORMAL RGB(3, 27, 3)
#define ORAS_DOWSING_COLOR_FAST   RGB(31, 31, 5)
#define ORAS_DOWSING_COLOR_FASTER RGB_RED
...
{
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 0),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 1),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 2),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 3),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 4),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 5),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 6),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 7),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingBrendan, 2, 4, 8),
};

static const struct SpriteFrameImage sPicTable_ORASDowsingMay[] =
{
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 0),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 1),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 2),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 3),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 4),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 5),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 6),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 7),
    overworld_frame(gFieldEffectObjectPic_ORASDowsingMay, 2, 4, 8),
};
...
void ResumeORASDowseFieldEffect(void)
{
    if (IsORASDowsingActive())
    {
        if (TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_BIKE | PLAYER_AVATAR_FLAG_SURFING | PLAYER_AVATAR_FLAG_UNDERWATER))
            EndORASDowsing();
        else
            StartORASDowseFieldEffect();
    }
...
u32 FldEff_ORASDowsing(void)
{
    ...
    FlagSet(FLAG_ORAS_DOWSING_ACTIVE);
    ...
    if (spriteId != MAX_SPRITES)
    {
        ...
        if (palNum != 0xFF)
        {
            ...
            UpdateSpritePaletteWithTime(palNum);
        }
        else
        {
            FlagClear(FLAG_ORAS_DOWSING_ACTIVE);
...
static void UpdateORASDowsingFieldEffect(struct Sprite *sprite)
{
    ...
    if (ORAS_DOWSING_SOUNDS && sprite->sDowseState == ORASD_WIGGLE_FASTER && playerObj->heldMovementFinished != FALSE)
    {
...
void UpdateDowseState(struct Sprite *sprite)
{
    ...
    if (ItemfinderCheckForHiddenItems(gMapHeader.events, sprite->data) == TRUE)
    {
        s8 distX = sprite->tItemDistanceX;
        s8 distY = sprite->tItemDistanceY;
        u8 directionToItem = DIR_NONE;
...
{
    s16 distance;
    u16 color = ORAS_DOWSING_COLOR_NONE;
    ...
    case 1:
        ...
        {
            color = ORAS_DOWSING_COLOR_FASTER;
            sprite->sDowseState = ORASD_WIGGLE_FASTER;
            break;
        }
    case 2:
        color = ORAS_DOWSING_COLOR_FAST;
        sprite->sDowseState = ORASD_WIGGLE_FAST;
        break;
    case 3:
    case 4:
        color = ORAS_DOWSING_COLOR_NORMAL;
        sprite->sDowseState = ORASD_WIGGLE_NORMAL;
        break;
    case 5:
    case 6:
    case 7:
        color = ORAS_DOWSING_COLOR_SLOW;
        sprite->sDowseState = ORASD_WIGGLE_SLOW;
        break;
    }

    if (ORAS_DOWSING_SOUNDS && sprite->sDowseState != sprite->sPrevDowseState)
    {
        sprite->sSoundTimer = 0;
        PlayDowseSound(sprite->sDowseState);
    }

    FillPalette(color, (OBJ_PLTT_ID(IndexOfSpritePaletteTag(FLDEFF_PAL_TAG_ORAS_DOWSE)) + ORAS_DOWSING_COLOR_PAL), PLTT_SIZEOF(1));
...
{
    sprite->sDowseState = ORASD_WIGGLE_NONE;
    FillPalette(ORAS_DOWSING_COLOR_NONE, (OBJ_PLTT_ID(IndexOfSpritePaletteTag(FLDEFF_PAL_TAG_ORAS_DOWSE)) + ORAS_DOWSING_COLOR_PAL), PLTT_SIZEOF(1));
    UpdateSpritePaletteWithTime(IndexOfSpritePaletteTag(FLDEFF_PAL_TAG_ORAS_DOWSE));
}
...
bool8 IsORASDowsingActive(void)
{
    return FlagGet(FLAG_ORAS_DOWSING_ACTIVE);
}
