...
static EWRAM_DATA u16   spriteIdGrayPalette[PARTY_SIZE] = {};
...
static void LoadMonIconGrayPalette(u8 partyId);
static void TintPartyMonIcons(u8 tm);
static void DestroyPartyMonIcons(void);
void CB2_ReturnToTMCaseFromPartyMenu(void);
...
void CB2_ReturnToTMCaseFromPartyMenu(void)
{
    InitTMCase(TMCASE_FROMFIELD, CB2_ReturnToField, 0xFF);
}

#define TM_CASE_MON_ICON_GRAY_PAL_TAG (TM_CASE_TM_TAG + 1)
...
static void DrawPartyMonIcons(void)
{
    ...
    for (i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        //create icon sprite
        spriteIdData[i] = CreateMonIcon2(species, SpriteCb_MonIcon, icon_x, icon_y, 1, otId, personality, TRUE);
        ...
        LoadMonIconGrayPalette(i);
    }
}

static void LoadMonIconGrayPalette(u8 partyId)
{
    u8 paletteNum = AllocSpritePalette(TM_CASE_MON_ICON_GRAY_PAL_TAG + partyId);

    if (paletteNum == 0xFF)
    {
        spriteIdGrayPalette[partyId] = spriteIdPalette[partyId];
        return;
    }

    spriteIdGrayPalette[partyId] = paletteNum;
    LoadPalette(&gPlttBufferUnfaded[OBJ_PLTT_ID(spriteIdPalette[partyId])], OBJ_PLTT_ID(paletteNum), PLTT_SIZE_4BPP);
    TintPalette_GrayScale2(&gPlttBufferUnfaded[OBJ_PLTT_ID(paletteNum)], 16);
    CpuCopy16(&gPlttBufferUnfaded[OBJ_PLTT_ID(paletteNum)], &gPlttBufferFaded[OBJ_PLTT_ID(paletteNum)], PLTT_SIZE_4BPP);
}
...
static void TintPartyMonIcons(u8 tm)
{
    ...
    for (i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        {
            gSprites[spriteIdData[i]].oam.paletteNum = spriteIdGrayPalette[i];
        }
        else
        {
...
static void DestroyPartyMonIcons(void)
{
    ...
    for (i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        FreeSpritePaletteByTag(TM_CASE_MON_ICON_GRAY_PAL_TAG + i);
    }
    FreeMonIconPalettes();
}
