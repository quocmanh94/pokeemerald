...
extern const u8 gText_PutTMHMInTMCase[];

static const u8 sLocalizationEnglish_Accuracy[] = _("accuracy");
static const u8 sLocalizationEnglish_Evasiveness[] = _("evasiveness");
static const u8 sLocalizationEnglish_PutTMHMInTMCase[] = _(
    "{PLAYER} put away the {STR_VAR_2}\n"
    "in the TM CASE."
);

static const u8 *FindSpecialTranslation(const u8 *string)
{
    if (string == gStatNamesTable[STAT_ACC])
        return sLocalizationEnglish_Accuracy;
    if (string == gStatNamesTable[STAT_EVASION])
        return sLocalizationEnglish_Evasiveness;
    if (string == gText_PutTMHMInTMCase)
        return sLocalizationEnglish_PutTMHMInTMCase;
...
const u8 *GetLocalizedString(const u8 *string)
{
    ...
    translation = FindSpecialTranslation(string);
    if (translation == NULL)
